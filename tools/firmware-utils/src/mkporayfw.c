 ...
 * Copyright (C) 2103 <admin@openschemes.com>
 *
...
"  -x              extract combined kernel and rootfs while inspecting (requires -i)\n"
"  -h              show this screen\n"
	);
...
static int inspect_fw(void)
{
	...
	/* Cannot use network order function because checksum is not word-aligned */
	file_checksum = (buf[inspect_info.file_size - 1] << 8) | buf[inspect_info.file_size - 2];
	...
	if (extract) {
		...
		if (ofname == NULL) {
			...
			sprintf(filename, "%s-firmware", inspect_info.file_name);
		} else {
			filename = ofname;
		}
